...
locals {
  ...
  next_origin_acl_version = try(
    data.tencentcloud_teo_origin_acl.coder.origin_acl_info[0].next_origin_acl[0].version,
    null,
  )
  ...
  origin_protection_ready = length(local.edgeone_origin_ipv6_cidrs) > 0
}

resource "kubernetes_manifest" "coder_origin_policy" {
  count = local.origin_protection_ready ? 1 : 0
...
resource "kubernetes_manifest" "coder_wildcard_dns" {
  ...
  depends_on = [kubernetes_manifest.coder_origin_policy]
}

resource "tencentcloud_teo_confirm_origin_acl_update_operation" "coder" {
  count = local.next_origin_acl_version != null ? 1 : 0
